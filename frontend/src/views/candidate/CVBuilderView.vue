  ...
  <div v-if="step === 'extracting'" class="flex flex-col items-center justify-center min-h-[calc(100vh-48px)] p-6">
    <div class="max-w-sm w-full space-y-8 text-center">
      <div class="flex justify-center">
        <div class="h-16 w-16 rounded-2xl flex items-center justify-center" :class="extractError ? 'bg-destructive/10 border border-destructive/20' : 'bg-primary/10 border border-primary/20'">
          <span v-if="extractError" class="mdi mdi-alert-circle-outline text-4xl text-destructive" />
          <span v-else class="mdi mdi-loading mdi-spin text-4xl text-primary" />
        </div>
      </div>
      <div>
        <h2 class="text-xl font-bold font-display mb-1 text-foreground">
          {{ extractError ? 'Extraction Failed' : 'Extracting CV Data' }}
        </h2>
        <p v-if="extractError" class="text-sm text-destructive">{{ extractError }}</p>
        <p v-else class="text-sm text-muted-foreground">{{ EXTRACT_STEPS[Math.min(extractStep, EXTRACT_STEPS.length - 1)] }}</p>
      </div>
      <div v-if="!extractError" class="space-y-2.5 text-left">
        <div
          v-for="(label, i) in EXTRACT_STEPS"
          :key="i"
          class="flex items-center gap-2.5 text-sm transition-all"
          :class="i < extractStep ? 'text-foreground' : i === extractStep ? 'text-muted-foreground' : 'text-white/20'"
        >
