  ...
  <div class="p-6 space-y-6">
    ...
    <div class="flex items-start justify-between flex-wrap gap-3">
      <div>
        ...
        <p class="text-sm mt-0.5 text-muted">{{ filtered.length }} application{{ filtered.length !== 1 ? 's' : '' }} tracked</p>
      </div>
      <div class="flex items-center gap-2 flex-wrap">
        ...
        <div class="flex items-center rounded-lg p-0.5 gap-0.5 border border-border bg-secondary">
          <button
            ...
            :class="['h-7 w-7 rounded-md flex items-center justify-center transition-colors focus-visible:ring-2 focus-visible:ring-primary', viewMode === mode ? 'bg-white/10 text-foreground' : 'text-muted hover:bg-white/5']"
          >
        ...
        <button
          ...
          class="flex items-center gap-1.5 h-9 px-4 rounded-lg text-sm font-medium border border-border text-foreground hover:bg-white/5 focus-visible:ring-2 focus-visible:ring-primary transition-colors"
        >
    ...
    <div class="flex items-center gap-3 flex-wrap">
      <div class="relative flex-1 min-w-[180px] max-w-sm">
        <span class="mdi mdi-magnify absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted" />
        <input
          v-model="search"
      ...
      <select
        ...
        class="input-field w-auto input-field-sm px-3"
      >
    ...
    <div v-else-if="viewMode === 'list'" class="space-y-2">
      <div
        ...
        :key="app.id"
        class="flex items-center gap-4 p-4 rounded-xl border border-border bg-card transition-colors hover:bg-card/80"
      >
        <div class="flex h-10 w-10 items-center justify-center rounded-lg shrink-0 bg-secondary">
          <span class="mdi mdi-domain text-lg text-muted" />
        </div>
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-3 mt-1 text-xs flex-wrap text-muted">
          ...
          <p v-if="app.notes" class="text-xs mt-1 truncate text-muted">{{ app.notes }}</p>
        </div>
      ...
      <div v-if="filtered.length === 0" class="flex flex-col items-center justify-center py-16 gap-3 text-muted">
    ...
    <div v-else-if="!loading" class="overflow-x-auto -mx-6 px-6 pb-4">
      <div class="flex gap-3 min-w-max">
        <div v-for="col in allStatuses" :key="col" class="flex flex-col gap-2 w-56 flex-shrink-0">
          <div class="flex items-center justify-between px-1">
            <span class="text-xs font-semibold uppercase tracking-wider" :style="{ color: statusColor(col) }">{{ statusLabels[col] }}</span>
            <span class="flex h-5 min-w-5 items-center justify-center rounded-full text-[10px] font-bold px-1.5 bg-secondary text-muted">
          ...
          <div class="space-y-2 min-h-16">
            <div
              ...
              :key="app.id"
              class="rounded-xl border border-border bg-card p-3 space-y-2 transition-colors hover:bg-card/80"
            >
              <div class="flex items-start justify-between gap-2">
                <div class="flex-1 min-w-0">
                  <p class="text-sm font-medium leading-snug text-foreground">{{ app.job }}</p>
                  <p class="text-xs mt-0.5 text-muted">{{ app.company }}</p>
                </div>
                <button @click="openEdit(app)" class="h-5 w-5 rounded flex items-center justify-center hover:bg-white/5 shrink-0 focus-visible:ring-2 focus-visible:ring-primary">
                  <span class="mdi mdi-pencil-outline text-xs text-muted" />
                </button>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-[10px] text-muted">{{ formatDate(app.date) }}</span>
                <select
                  :value="app.status"
                  @change="handleStatusChange(app.id, ($event.target as HTMLSelectElement).value as AppStatus)"
      ...
      <div class="w-full max-w-lg rounded-2xl border border-border bg-card p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between">
          ...
          <button @click="dialogOpen = false" class="h-7 w-7 rounded flex items-center justify-center hover:bg-white/5 focus-visible:ring-2 focus-visible:ring-primary">
            <span class="mdi mdi-close text-muted" />
          </button>
        </div>
        ...
        <div class="flex justify-end gap-2">
          <button @click="dialogOpen = false" class="h-9 px-4 rounded-lg text-sm border border-border text-muted hover:bg-white/5 focus-visible:ring-2 focus-visible:ring-primary">Cancel</button>
          <button
            @click="handleSave"
    ...
    >
      <div class="w-full max-w-lg rounded-2xl border border-border bg-card p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        ...
        <div class="max-h-48 overflow-y-auto rounded-lg border border-border">
          ...
          >
            <span class="font-medium shrink-0 w-5 text-right text-muted">{{ i + 1 }}</span>
        ...
        <div class="flex justify-end gap-2">
          <button @click="importDialogOpen = false" class="h-9 px-4 rounded-lg text-sm border border-border text-muted hover:bg-white/5 focus-visible:ring-2 focus-visible:ring-primary">Cancel</button>
          <button
...
export type AppStatus = 'WISHLIST' | 'APPLIED' | 'PHONE_SCREEN' | 'INTERVIEW' | 'OFFER' | 'REJECTED' | 'WITHDRAWN';
...
const allStatuses: AppStatus[] = ['WISHLIST', 'APPLIED', 'PHONE_SCREEN', 'INTERVIEW', 'OFFER', 'REJECTED', 'WITHDRAWN'];

const statusColorMap: Record<AppStatus, string> = {
  WISHLIST: 'var(--color-muted)',
...
const statusBgMap: Record<AppStatus, string> = {
  WISHLIST: 'hsl(var(--color-muted-raw) / 0.1)',
  APPLIED: 'hsl(var(--color-info-raw) / 0.1)',
  PHONE_SCREEN: 'rgba(167,139,250,0.1)',
  INTERVIEW: 'hsl(var(--color-warning-raw) / 0.1)',
  OFFER: 'hsl(var(--color-success-raw) / 0.1)',
  REJECTED: 'hsl(var(--color-destructive-raw) / 0.1)',
  WITHDRAWN: 'hsl(var(--color-muted-raw) / 0.1)',
};
...
async function handleCsvImport() {
  ...
  const mapped = csvPreview.value.map((row: any) => ({
    ...
    status: (row.status || 'WISHLIST').toUpperCase().replace(/ /g, '_'),
